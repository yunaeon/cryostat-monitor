...
# Program name
WATCHED="grips_monitor_mode.py"
WATCHED_PATH="/home/grips/cryomon/"
...
LOG=/home/grips/cryomon/logs/monitor_$datetime
WLOG=/home/grips/cryomon/logs/${WATCHED}Watchdog.log

# Check if we have exactly one process running
COUNT=`ps -ef | grep python | grep ${WATCHED} | wc -l`

if [ ${COUNT} -eq 0 ]; then
  # No instance is running - we start it
  echo "ERROR: ${WATCHED} not running! Starting it!" | ts >> ${WLOG}
  echo "creating log: ${LOG}" | ts >> ${WLOG}
  python ${WATCHED_PATH}${WATCHED} ${CMDLINE} &> ${LOG} &
  LASTPID=$!
  ...
  ps -ef | grep python | grep ${WATCHED} | while read LINE; do
